  ...
  const loadReference = useCallback(async () => {
    try {
      ...
      if (result.success && result.references) {
        const foundReference = result.references.find(ref => ref.id === parseInt(referenceId));
        
        if (foundReference && foundReference.images && foundReference.images.length > 0) {
          console.log(`✅ Loaded reference: ${foundReference.title}`);
          setReference(foundReference);
        } else {
          console.log('❌ Reference not found or has no images');
          navigate('/references');
        }
      } else {
        console.log('❌ Failed to load references');
        navigate('/references');
      }
    } catch (error) {
